...
export function Summary({items = [], onOrderSubmit}) {
    ...
    const handleAddressInputCanged = (e) => setOrder({...order, address: e.target.value});
    const handlePostcodeInputChanged = (e) => setOrder({...order, postcode: e.target.value});

    const handleSubmit = (e) => {
        if(order.address === "" || order.email === "" || order.postcode == "") alert("입력값을 확인해주세요");
        else{
            onOrderSubmit(order);
        }
    }
        ...
        <React.Fragment>
            ...
            <form>
                ...
                <div className="mb-3">
                    ...
                    <input type="text" className="form-control mb-1" value={order.address} onChange={handleAddressInputCanged}
                           id="address"/>
                </div>
                <div className="mb-3">
